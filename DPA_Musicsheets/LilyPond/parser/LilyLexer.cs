...
namespace DPA_Musicsheets.LilyPond.parser
{
    public class LilyLexer
    {
        ...
        {
            string[] s_tokens = LilyTokenizer.Tokenize(source);
            ...
            foreach (string s_token in s_tokens)
            {
                ...
                foreach (KeyValuePair<string, TokenMatcher> kv in tokenMatchers)
                {
                    ...
                    {
                        hasAMatch = true;
                        tokens.Add(new t_LilypondToken(s_token, kv.Value, regex.Match(s_token)));
                        break;
                    }
                }
                if (!hasAMatch)
        ...
        public static string TokensToString(List<t_LilypondToken> tokens)
        {
            StringBuilder concatenated = new StringBuilder();
            foreach (t_LilypondToken token in tokens)
            {
